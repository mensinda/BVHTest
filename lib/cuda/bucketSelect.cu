...
void setToAllZero(uint *deviceVector, int length) { cudaMemset(deviceVector, 0, length * sizeof(uint)); }

// this function assigns elements to buckets
template <typename T>
__global__ void assignBucket(T *    d_vector,
                             int    length,
                             int    bucketNumbers,
                             double slope,
                             double minimum,
  ...
  for (int i = 0; i < (bucketNumbers / 1024); i++)
    if (index < bucketNumbers) sharedBuckets[i * 1024 + index] = 0;
  __syncthreads();
  ...
  for (int I = idx; I < length; I += stride) {
    ...
    for (i = idx; i < length; i += offset) {
      ...
      if (bucketIndex >= sbucketNums) { bucketIndex = sbucketNums - 1; }
  ...
  for (int i = 0; i < (bucketNumbers / 1024); i++)
    if (index < bucketNumbers) atomicAdd(&bucketCount[i * 1024 + index], sharedBuckets[i * 1024 + index]);
}

// this function reassigns elements to buckets
template <typename T>
__global__ void reassignBucket(T *          d_vector,
                               int *        bucket,
                               uint *       bucketCount,
                               const int    bucketNumbers,
                               const int    length,
                               const double slope,
  ...
  if (index < bucketNumbers) { sharedBuckets[index] = 0; }
  __syncthreads();
  ...
  // reading bucket counts from shared memory back to global memory
  if (index < bucketNumbers) { atomicAdd(&bucketCount[index], sharedBuckets[index]); }
}
...
T phaseTwo(T *d_vector, int length, int K, int blocks, int threads, double maxValue = 0, double minValue = 0) {
  ...
  assignBucket<<<numBlocks, threadsPerBlock, numBuckets * sizeof(uint)>>>(
      d_vector, length, numBuckets, slope, minValue, elementToBucket, d_bucketCount, offset);
  ...
  while ((Kbucket_count > 1) && (iter < 1000)) {
    ...
    if (maxValue - minValue > 0.0f) {
      ...
      setToAllZero(d_bucketCount, numBuckets);
      reassignBucket<<<numBlocks, threadsPerBlock, numBuckets * sizeof(uint)>>>(
          d_vector, elementToBucket, d_bucketCount, numBuckets, length, slope, maxValue, minValue, offset, Kbucket);
...
T phaseOne(T *d_vector, int length, int K, int blocks, int threads, int pass = 0) {
  ...
  if (K == length) { return maximum; }
  // Allocate memory to store bucket assignments

  CUDA_CALL_BS(cudaMalloc(&elementToBucket, size));

  // Allocate memory to store bucket counts
  size_t totalBucketSize = numBuckets * sizeof(uint);
  CUDA_CALL_BS(cudaMalloc(&d_bucketCount, totalBucketSize));
  uint *h_bucketCount = (uint *)malloc(totalBucketSize);
  ...
  assignBucket<<<numBlocks, threadsPerBlock, numBuckets * sizeof(uint)>>>(
      d_vector, length, numBuckets, slope, minimum, elementToBucket, d_bucketCount, offset);
  ...
  if (kthBucket != 0) { K = kthBucketCount - (kthBucketScanner - K); }
