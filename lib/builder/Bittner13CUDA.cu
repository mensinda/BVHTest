...
__device__ uint32_t findNode2(uint32_t _n, BVHPatch &_bvh) {
  ...
  while (lMin < HUGE_VALF) {
    ...
    uint64_t lCurrNode = _bvh.getSubset(lCurrNL);
    ...
    if ((lNewInduced + lSArea) < lBestCost && !_bvh.isLeaf(lCurrNode)) {
      lPQ_NL[lMinIndex + lStart] = _bvh.left(lCurrNode);
      lPQ_NL[lMaxIndex + lStart] = _bvh.right(lCurrNode);
...
extern "C" __global__ void kCalcCost(
  ...
  for (uint32_t i = index; i < _num; i += stride) {
    uint  lID   = _nID[i];
    float lSA   = _nodes.surfaceArea[lID];
    _costOut[i] = (lSA * lSA * lSA * (float)_nodes.numChildren[lID]) / (_sum[lID] * _min[lID]);
  }
}
...
GPUWorkingMemory allocateMemory(CUDAMemoryBVHPointer *_bvh, uint32_t _batchSize, uint32_t _numFaces) {
  ...
  CUBLeafSelect lSelector(_bvh->nodes);
